...
export class SVGAnalyzer {
  ...
  async analyze(): Promise<DesignSystemAnalysis> {
    ...
    analysis.complexity.average = files.length > 0 ? analysis.complexity.total / files.length : 0;
  ...
  private analyzeComponent(doc: Document, filename: string): SVGComponentAnalysis {
  ...
  private extractInteractions(doc: Document): string[] {
  ...
  private extractSections(doc: Document): string[] {
  ...
  private determineComponentType(doc: Document): string {
  ...
  private extractMetadata(doc: Document): Record<string, string> {
    ...
    for (const comment of commentNodes) {
      const content = comment.data.trim();
      if (content.startsWith('@')) {
        const [key, ...valueParts] = content.slice(1).split(':');
